...
@Service
public class SetUseThreadsCommand implements ICommand {
    private final GuildService guildService;

    @Autowired
    public SetUseThreadsCommand(GuildService guildService) {
    ...
    public void handle(CommandContext ctx) {
        ...
        if (content.length != 2) {
            ctx.sendError("Incorrect syntax, please use `!usethreads <true/false>`");
            return;
        }
        boolean flag;
        try {
        ...
        guildService.setUseThreads(ctx.getGuildId(), flag);
        ctx.reactPositive();
    }
    ...
    public List<String> getIdentifiers() {
        return Collections.singletonList("usethreads");
    }
}
