...
public class AddCarryoverForCommand implements ICommand {
    ...
    public void handle(CommandContext ctx) {
        ...
        if (!cachedSpreadSheetUtils.userIsSignedUpForMultipleSpreadsheets(ctx.getGuildId(), mentionedMembers.get(0).getUser().getId())) {
            ...
            damageStrategy.addCarryover(userID, sheetID, damage, ctx.getJDA());
            ctx.reactPositive();
            return;
        }
        String messageID = ctx.getMessage().getId();
        ctx.getChannel().sendMessage("This user is participating in multiple clans").setComponents(cachedSpreadSheetUtils.getSpreadsheetButtons(ctx.getGuildId(), "addcarryoverfor-" + messageID)).queue();
        ctx.reactWIP();
    }
